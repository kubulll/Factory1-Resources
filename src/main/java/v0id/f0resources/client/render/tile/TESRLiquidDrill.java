...
public class TESRLiquidDrill extends FastTESR<TileLiquidDrill>
{
    ...
    public TESRLiquidDrill()
    {
        try
        {
            model.load(Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("f0-resources", "models/block/liquid_drill.obj")).getInputStream());
            modelHead.load(Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("f0-resources", "models/block/liquid_drill_head.obj")).getInputStream());
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    @Override
    public void renderTileEntityFast(TileLiquidDrill te, double x, double y, double z, float partialTicks, int destroyStage, float partial, BufferBuilder buffer)
    {
        if (te.isCenter)
        {

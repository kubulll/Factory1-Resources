...
public class TESRLiquidDrillSlow extends TileEntitySpecialRenderer<TileLiquidDrill>
{
    public TESRLiquidDrillSlow()
    {
        try
        {
            TESRLiquidDrill.model.load(Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("f0-resources", "models/block/liquid_drill.obj")).getInputStream());
            TESRLiquidDrill.modelHead.load(Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("f0-resources", "models/block/liquid_drill_head.obj")).getInputStream());
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    @Override
    public void render(TileLiquidDrill te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {
        if (te.isCenter)
        {
            Minecraft.getMinecraft().renderEngine.bindTexture(F0RTextures.textureLiquidDrill);
            ...
            TESRLiquidDrill.model.putVertices(bb);
            ...
            if (!is.isEmpty() && is.getItem() instanceof ItemDrillHead)
            {
                ...
                for (WavefrontObject.Vertex vertex : TESRLiquidDrill.modelHead.getVertices())
                {

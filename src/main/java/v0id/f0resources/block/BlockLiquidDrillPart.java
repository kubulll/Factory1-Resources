...
public class BlockLiquidDrillPart extends Block
{
    public BlockLiquidDrillPart()
    {
        super(Material.IRON);
        this.setRegistryName(F0RRegistryNames.asLocation(F0RRegistryNames.liquidDrillPart));
    ...
    {
        return Item.getItemFromBlock(F0RBlocks.liquidDrillComponent);
    }

    @Override
    public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player)
    {
        return new ItemStack(F0RBlocks.liquidDrillComponent, 1, 0);
    }
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        {
            TileLiquidDrill tile = (TileLiquidDrill) worldIn.getTileEntity(pos);
            if (tile.centerPos != BlockPos.ORIGIN)
            {
                ...
                for (int dx = -1; dx <= 1; ++dx)
                {
                    for (int dz = -1; dz <= 1; ++dz)
                    {
                        ...
                        {
                            worldIn.setBlockState(at, F0RBlocks.liquidDrillComponent.getDefaultState());
                        }
                    }
                }
        ...
        if (tile instanceof TileLiquidDrill && ((TileLiquidDrill) tile).isCenter && !((TileLiquidDrill) tile).inventory.getStackInSlot(0).isEmpty())
        {
            InventoryHelper.spawnItemStack(worldIn, pos.getX(), pos.getY() + 1, pos.getZ(), ((TileLiquidDrill) tile).inventory.getStackInSlot(0).copy());
        }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        {
            TileLiquidDrill drill = (TileLiquidDrill) worldIn.getTileEntity(pos);
            if (drill.centerPos != BlockPos.ORIGIN)
            {
                ...
                if (tile instanceof TileLiquidDrill)
                {
                    ...
                    ((TileLiquidDrill) tile).outputPos = at;
